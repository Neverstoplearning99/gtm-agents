...
Learn how Claude Code's sandboxed bash tool provides filesystem and network isolation for safer, more autonomous agent execution when running GTM Agents workflows.
...
For GTM Agents users, sandboxing is especially useful when running orchestrators (such as `/campaign-orchestration:launch-campaign` or `/abm-orchestration:target-accounts`) that chain together many bash steps, scripts, and integrations.
...
- **Default writes behavior**: Read and write access to the current working directory and its subdirectories (this includes the `workspace/` folders used by GTM Agents — `leads/`, `campaigns/`, `content/`, etc.)
...
> 📝 **Note:** The built-in proxy enforces the allowlist based on the requested hostname and does not terminate or inspect TLS traffic. See [Security limitations](#security-limitations) for the implications of this design, and [Custom proxy configuration](#custom-proxy-configuration) if your threat model requires TLS inspection.
...
By default, if the sandbox cannot start (missing dependencies or unsupported platform), Claude Code shows a warning and runs commands without sandboxing. To make this a hard failure instead, set `sandbox.failIfUnavailable` to `true`. This is intended for managed deployments that require sandboxing as a security gate.
...
> ℹ️ Auto-allow mode works independently of your permission mode setting. Even if you're not in "accept edits" mode, sandboxed bash commands will run automatically when auto-allow is enabled. This means bash commands that modify files within the sandbox boundaries will execute without prompting, even when file edit tools would normally require approval.

### Configure sandboxing

Customize sandbox behavior through your `settings.json` file. See [Settings](https://docs.claude.com/en/settings#sandbox-settings) for the complete configuration reference.
...
The [claude-code repository](https://github.com/anthropics/claude-code/tree/main/examples/settings) includes starter settings configurations for common deployment scenarios, including sandbox-specific examples. Use these as starting points and adjust them to fit your needs.

---

## Recommended setup for GTM Agents

GTM Agents writes most outputs to the `workspace/` directory tree (see [`CLAUDE.md`](../CLAUDE.md) for the full layout). Because all of those folders sit inside the project root, the default sandbox already permits writes to them — you typically do not need to add `allowWrite` entries to run the standard plugins.

A reasonable starting `.claude/settings.json` for this repo:
...
Add the GTM tool domains you actually use (Salesforce, HubSpot, LinkedIn, Apollo, Gmail, Notion, Airtable, etc.) to `allowedDomains` only as the workflows that need them are introduced.
...
- **Development containers**: Use with dev containers for additional isolation
...
The sandbox runtime is available as an open source npm package for use in your own agent projects. This enables the broader AI agent community to build safer, more secure autonomous systems. It can also be used to sandbox other programs you may wish to run. For example, to sandbox an MCP server you could run:
...
- **Computer use**: when Claude opens apps and controls your screen, it runs on your actual desktop rather than in an isolated environment. Per-app permission prompts gate each application.

---

## See also

- [Claude Code Cheat Sheet](claude-code-cheat-sheet.md) — Setup and everyday tips
- [Usage Guide](usage-guide.md) — Commands and workflows
- [SECURITY.md](../SECURITY.md) — Reporting vulnerabilities in GTM Agents
- [Claude Code: Security](https://docs.claude.com/en/security) — Comprehensive security features and best practices
- [Claude Code: Permissions](https://docs.claude.com/en/permissions) — Permission configuration and access control
- [Claude Code: Settings](https://docs.claude.com/en/settings) — Complete configuration reference
